...
|                                                                     |- Pathways              |                          |
+---------------------------------------------------------------------+------------------------+--------------------------+
| `PTMInt` <https://ptmint.sjtu.edu.cn/>_                             |- Interactions          | 4/17/2024                |
+---------------------------------------------------------------------+------------------------+--------------------------+
...
Appending Annotations to PTM Dataframes
---------------------------------------
While appending these annotations directly to the PTM dataframes generated by the `project` module is not necessary for the analyis modules described in the analysis gallery, you may wish to append this information directly. To append this information to either the `spliced_ptms` or `altered_flanks` dataframe, use the :func:`annotate_ptms() <ptm_pose.annotate.annotate_ptms>` function.

.. code-block:: python

    from ptm_pose import annotate

    spliced_ptms = annotate.annotate_ptms(spliced_ptms, annot_type = 'All', combine_similar = True, report_success = True)

You can restrict this function to only append information of certain types with `annot_type` parameter, or change which databases are used by setting the boolean parameters for each database (e.g. `phosphositeplus = False` to exclude PhosphoSitePlus annotations). If you ever need to check what annotations are available, you can use the :func:`get_available_gmt_annotations <ptm_pose.annotate.get_available_gmt_annotations>` function to see what annotations are available.



You can also append information individually using the :func:`append_from_gmt <ptm_pose.annotate.append_from_gmt>` function, indicating the desired database and annotation type. For example, to append PhosphoSitePlus kinase substrate information to the `spliced_ptms` dataframe, you would use the following code:
