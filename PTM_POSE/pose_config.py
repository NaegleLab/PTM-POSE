...
annotation_col_dict = {'PhosphoSitePlus': {'Function':'PSP:ON_FUNCTION', 'Process':'PSP:ON_PROCESS', 'Disease':"PSP:Disease_Association", 'Kinase':'PSP:Kinase', 'Interactions': 'PSP:ON_PROT_INTERACT'},
...
def download_background(annotation_type = 'PhosphoSitePlus', database = 'Function', collapsed = False):
    fname = f'{database}_{annotation_type}_collapsed.csv' if collapsed else f'{database}_{annotation_type}.csv'
    background = pd.read_csv(resource_dir + '/background_annotations/'+fname,index_col = 0)
    return background
